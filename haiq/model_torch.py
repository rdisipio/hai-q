...
class QLSTM(nn.Module):
    def __init__(self, 
                ...
                return_state=False,
                backend="default.qubit.autograd",
                interface='torch',
        ...
        print(f"Interface: {self.interface}, backend: {self.backend}, diff_method: {self.diff_method}")
...
class HaikuLM(nn.Module):
    def __init__(self,
                embed_dim: int,
                vocab_size: int,
                hidden_dim: int,
                n_qubits: int=4,
                backend: str='default.qubit.autograd',
                interface: str='torch',
                diff_method='backprop',):
        super(HaikuLM, self).__init__()
        self.embed = nn.Embedding(vocab_size, embed_dim)
        self.lstm = QLSTM(embed_dim, hidden_dim, n_qubits=n_qubits, 
                            interface=interface, backend=backend, diff_method=diff_method)
        self.hidden2tag = nn.Linear(hidden_dim, vocab_size)

    def forward(self, sentence):
        embeds = self.embed(sentence)
        lstm_out, _ = self.lstm(embeds)
        last_timestamp = lstm_out[:, -1, :]
        logits = self.hidden2tag(last_timestamp)
        return logits
